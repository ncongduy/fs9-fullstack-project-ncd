...
describe('book controller', () => {
  ...
  it('should not delete an existing book with user authorization', async () => {
    ...
    res = await request(app).delete(`/api/v1/books/${bookId}`).set({ Authorization: userToken })
  ...
  it('should delete an existing book', async () => {
    ...
    res = await request(app).delete(`/api/v1/books/${bookId}`).set({ Authorization: adminToken })
